...
test('syncGateContract builds two staging LOCALs + a barrier (no parcels/roles)', () => {
  const c = syncGateContract('g1', { x: 5, y: 5 }, 700, 9999)
  expect(c.type).toBe('SYNC_GATE')
  expect(c.status).toBe('PROPOSED')
  expect(c.lockParcels).toBeUndefined()
  expect(c.steps).toEqual([
    { kind: 'LOCAL', agent: 'liaison', goal: { kind: 'ODD_ROW', hint: { x: 5, y: 5 } }, post: 'l_staged' },
    { kind: 'LOCAL', agent: 'courier', goal: { kind: 'ODD_ROW', hint: { x: 5, y: 5 } }, post: 'c_staged' },
    { kind: 'BARRIER', needs: ['l_staged', 'c_staged'] },
  ])
})

test('advance returns "gated" once a SYNC_GATE barrier is released (not "done")', () => {
  const c: Contract = { ...syncGateContract('g1', { x: 5, y: 5 }, 700, 9999), status: 'ACTIVE', posted: { l_staged: true, c_staged: true } }
  expect(advance(c, 'liaison', { x: 5, y: 5 })).toEqual({ kind: 'gated' })
})

test('advance stages a SYNC_GATE onto an odd row (§8.5): navigate off-parity, post on any odd row', () => {
  const c: Contract = { ...syncGateContract('g1', { x: 5, y: 5 }, 700, 9999), status: 'ACTIVE', posted: {} }
  expect(advance(c, 'liaison', { x: 0, y: 0 })).toEqual({ kind: 'navigate', to: { x: 5, y: 5 } }) // even row → route to odd-row hint
  expect(advance(c, 'liaison', { x: 2, y: 3 })).toEqual({ kind: 'post', milestone: 'l_staged' }) // any odd row credits
})
...
function activeGate(): ContractRuntime {
  ...
  rt.applyMsg({ kind: 'propose', contract: syncGateContract('g1', { x: 5, y: 5 }, 700, 9999) }, 'courier')
  return rt
}
